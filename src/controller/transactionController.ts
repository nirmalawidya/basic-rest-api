    ...
    try {
        ...
        if(req.query.start_date && req.query.end_date){
            allTransaction = allTransaction
                .filter(trans =>
                    trans.order_date >= start_date // lebih dari =
                    &&
                    trans.order_date <= end_date // kurang dari =
                )
        }
...
const deleteTransaction = async (
    ...
    try {
        ...
        if (!findTransaction) { // ! artinya not(tidak)
            return res
                .status(400)
                .json({
        ...
        /** hapus detail transaksi dulu, krn
         * detail transaksi adalah tabel yang bergantung pada 
         * tabel transaksi
         */
        ...
        return res.status(200)
            .json({
                message: `Transaction has ben removed`
            })
    } catch (error) {
        return res
            .status(500)
